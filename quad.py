...
def curve(t):
    ...
    # paper shape
    return np.array([1.1 + (1 + 0.2 * cos(4*t)) * cos(t),
...
def dcurve_dt(t):
    ...
    #     ])
    return np.array([-(1 + 0.2 * cos(4*t)) * sin(t) - 0.8 * sin(4*t) * cos(t),
...
class QuadratureInfo:
    def __init__(self, nintervals):
        ...
        self.curve_nodes = (curve(self.par_nodes.reshape(-1))
                            .reshape(2, nintervals, -1))
        self.curve_deriv = (dcurve_dt(self.par_nodes.reshape(-1))
                            .reshape(2, nintervals, -1))
        ...
        self.curve_weights = (self.curve_speed * ref_weights *
                              par_intv_length / 2)
        ...
        plt.plot(
            ...
            self.curve_nodes[1].reshape(-1), 'g')
