...
som_rhs = som_sp(domain.curve_nodes[0], domain.curve_nodes[1], xs,
                 ys)
...
soln1 = bvp(npanels, k, domain, alpha, qbx_exp_slp, qbx_exp_dlp, rhs,
            som_sp=som_sp, som_dp=som_dp, imgs=imgs)
...
num_test = eval_target(test_targets, domain.curve_nodes, domain.curve_weights,
                       domain.normals, soln1, sp, dp, alpha, som_sp=som_sp,
                       som_dp=som_dp, imgs=imgs).reshape(tg_size, -1)
v = np.linspace(-.15, 0.15, 200, endpoint=True)
plt.figure(1)
